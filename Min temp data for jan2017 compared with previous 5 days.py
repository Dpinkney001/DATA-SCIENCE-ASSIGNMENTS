...
def previousfiveDayAve(minVals, index):
    ...
    if (index == 1):
        ...
        for dayCounter in range(index,32):
            totalOfTemps += minVals[dayCounter]
            ...
            dayCounter += 1
        fivedayAve = (totalOfTemps) / len(minVals) 
...
def main():
    ...
    for index in range(1,32):
        ...
        output = previousfiveDayAve(minVals, index)
        dayArray = fiveDays(minVals, index)
        plt.plot(output, dayArray, color='r', label="Minimaltemp")
        plt.title("Daily NYC Temps for Jan 2017")
        plt.xlabel('Day')
        plt.ylabel('Degrees')
        plt.legend(loc = 2,fontsize = 'x-small')
